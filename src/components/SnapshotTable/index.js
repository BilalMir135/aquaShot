...
export default function StickyHeadTable() {
  ...
  function formatSnapshotBalance(snapshots) {
    return snapshots.map((snapshot) => {
      return {
        ...snapshot,
        xioBalance: parseFloat(web3.utils.fromWei(snapshot.xioBalance)).toFixed(
          2
        ),
        xioAppBalance: parseFloat(
          web3.utils.fromWei(snapshot.xioAppBalance)
        ).toFixed(2),
        uniBalance: parseFloat(web3.utils.fromWei(snapshot.uniBalance)).toFixed(
          2
        ),
        xioAppBalance2: parseFloat(
          web3.utils.fromWei(snapshot.xioAppBalance2)
        ).toFixed(2),
      };
    });
  }
